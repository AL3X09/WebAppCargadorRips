...
minday = new Date(currentDate1.getFullYear() - 1, currentDate1.getMonth() - 2, 1);
maxday = new Date(currentDate2.getFullYear(), currentDate2.getMonth(), 0);
...
for (var d = minday; d <= maxday; d.setMonth(d.getMonth() + 1)) {
  ...
  reco = (d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + "0").split("-");
...
function inicilizoDatepicker() {
  ...
  var $input = $("#fechaInicio").pickadate({
    ...
    hiddenName: true,
    min: new Date(currentDate1.getFullYear() - 1, currentDate1.getMonth() - 1, 1),
    max: maxday,
    disable: daysOfYear,
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(data, function (i, v) {
        ...
        option = $('<option></option>').attr("value", "tipo" + v.codigo).text(v.descripcion);
        $("#tipoUsuario").append(option);
      });
      //console.log(option);

      $("#tipoUsuario").material_select();
    }
  }).fail(function (jqXHR, textStatus, errorThrown) {
    console.error(textStatus, errorThrown); // Algo fallo
    swal(
...
function callCategoria(objeto) {
  ...
  $.ajax({
    ...
    success: function (data) {
      var option = $('<option></option>').attr("value", "").text("Seleccione...");
      $.each(data, function (i, v) {
        option = $('<option></option>').attr("value", v.codigo).text(v.nombre);
    ...
    swal(
      textStatus,
      'Hay un problema al traer los datos comunícate con el administrador ',
      'error',
      setTimeout(function () {
      }, 2000)
    )
